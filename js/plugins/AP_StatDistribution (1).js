...
Game_Actor.prototype.resetPoints = function() {
	const refund = 0;
	for (const i = 0; i < this._spentPoints.length; i++) {
	...
	if (refund > 0) {
	this.addDistributionPoints(refund);
	for (const i = 0; i < this._APParams.length; i++) {
...
PluginManager.registerCommand(pluginName, "ResetPoints", actor => {
    $gameActors.actor(actor).resetPoints();
});
