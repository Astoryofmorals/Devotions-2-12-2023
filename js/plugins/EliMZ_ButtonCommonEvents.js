...
into proper functions: One to call the menu and the other to 
...
@desc Remove a common event of a key.
    ...
    @text Restore Key
    @type boolean
    @desc If true, and that key has a previous key assign to it, the game will restore it to the original function.
    @default true
...
Eli.ButtonCommonEvents = {
    ...
    initParameters(){
        const parameters = PluginManager.parameters("EliMZ_ButtonCommonEvents")
...
/* ----------------------------- PLUGIN COMMANDS ---------------------------- */
    ...
    executePluginCommandMV(args){
        ...
        if(this[cmd]) {
            const mzArgs = {key: button, commonEvent: id_restore, restoreKey: id_restore, isParallel: isParallel, overwrite: overwrite}
            this[cmd](mzArgs)
        }
    },
