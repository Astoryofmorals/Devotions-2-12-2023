...
/*:
 * @plugindesc (v.1.0.1)[BASIC] Simple quests system
 ...
 * ---------------------------------------------------------------------------
 *
  * This is BASIC plugin version and have some restrictions: *    - You can set only ONE (active) quest to tracking on map *    - Only 4 (max) tasks per quest *    - Only 4 (max) descriptions per quest *    - Not have journal notifications *  *  PRO version of plugin don't have this restrictions!
...
(function() {
  ...
  _.sqGetAllQuests = function() {
    if (this.sqQuests == null) {
      this.sqQuests = PKD_SQS.PP.createAllQuests();
    }
    return this.sqQuests;
  };
  _.sqGetAllPoints = function() {
    if (this.sqPoints == null) {
      this.sqPoints = PKD_SQS.PP.createAllPoints();
    }
    return this.sqPoints;
  };
})();
...
SQSQuestArrow = class SQSQuestArrow extends Sprite {
  ...
  updateRotationToTarget() {
    return this.rotation = Math.atan2(this.y - this.tarEv.screenY(), this.x - this.tarEv.screenX());
  }
...
SQS_Keep = class SQS_Keep {
  ...
  addQuest(questId) {
    if (!this.isAddedQuest(questId)) {
      return this._visibleQuests.push(questId);
    }
  }
...
SQSM.playerCurrentQuests = function() {
  var keep;
  keep = $gamePlayer.sqGetQuestsData();
  return SQSM.quests().filter(function(q) {
...
SQS_Quest = class SQS_Quest {
  ...
  getActiveDescription() {
    ...
    return this.getDescription(index);
  }
...
Window_SQSQuestsList = class Window_SQSQuestsList extends Window_Selectable {
  ...
  makeItemList() {
    if (this._group === 0) {
      this._data = SQSM.playerCurrentQuests();
    } else {
      this._data = SQSM.playerCompletedQuests();
    }
  }
...
(function() {  //╒═════════════════════════════════════════════════════════════════════════╛
  ...
  _._prepareParams = function() {
    var _activeQuestMargins, _cursorMargins;
...
(function() {
  ...
  _._showActiveQuestData = function() {
    ...
    this.qiDesc.setText(JsonEx.parse(this.activeQuestData.getActiveDescription()));
...
//Plugin PKD_SimpleQuestSystem automatic build by PKD PluginBuilder 1.9.2 20.06.2021
